...
class DynamicConv(nn.Module):
    ...
    def forward(self, param_feature, input_feature):
        ...
        # shape (bs * num_proposal, self.num_params_in + self.num_params_out)
        param_in = parameters[:, :self.num_params_in].view(
            -1, self.in_channels, self.feat_channels)
        param_out = parameters[:, -self.num_params_out:].view(
            -1, self.feat_channels, self.out_channels)

        # input_feature has shape (num_all_proposals, H*W(7*7), in_channels)
        ...
        # param_out has shape (num_all_proposals, feat_channels, out_channels)
        features = torch.bmm(features, param_out)
        features = self.norm_out(features)
        features = self.activation(features)  # (num_all_proposals, h*w, out_channels)

        features = features.flatten(1)  # (num_all_proposals, h*w*out_channels)
