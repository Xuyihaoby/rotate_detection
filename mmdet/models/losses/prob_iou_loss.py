# from https://github.com/lzh420202/FCOSR/blob/master/mmdet/models/losses/prob_iou_loss.py
...
class ProbiouLoss(nn.Module):
    ...
    def forward(self, loc_p, loc_t, weight, avg_factor=None):
        if avg_factor is None:
            avg_factor = 1.0
        loss = self.loss_weight * torch.sum(probiou_loss(loc_p, loc_t, self.eps, self.mode) * weight.mean(1))[None] / avg_factor
        return loss
