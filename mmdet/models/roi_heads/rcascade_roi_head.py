...
class RCascadeRoIHead(BaseRoIHead, RBBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(RCascadeRoIHead, self).__init__(
    ...
    def forward_train(self,
        ...
        Args:
            ...
            proposals (list[Tensors]): list of region proposals.[x1, y1, x2, y2, scores]
            ...
            if i < self.num_stages - 1:
                ...
                with torch.no_grad():
                    roi_labels = torch.where(
                        ...
                        bbox_results['cls_score_h'][:, :-1].argmax(1),
                        roi_labels)
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False, obb=True, submission=True):
        ...
        for i in range(self.num_stages):
            ...
            if isinstance(bbox_pred_h, torch.Tensor):
                ...
                bbox_pred_h = self.bbox_head[i].bbox_pred_split(
            ...
            if i < self.num_stages - 1:
                ...
                bbox_label_h = [s[:, :-1].argmax(dim=1) for s in cls_score_h]
                rois = torch.cat([
                    self.bbox_head[i].regress_by_class(rois[j], bbox_label_h[j],
                                                       bbox_pred_h[j],
                                                       img_metas[j])
                    for j in range(num_imgs)
                ])
        ...
        # horizon
        cls_score_h = [
            sum([score_h[i] for score_h in ms_scores_h]) / float(len(ms_scores_h))
