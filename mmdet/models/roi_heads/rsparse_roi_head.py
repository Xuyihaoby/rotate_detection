...
from mmdet.core import rbbox2result, rbbox2roi, CV_L_Rad2LE_DEF_TORCH
...
class RSparseRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
        ...
        super(RSparseRoIHead, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _bbox_forward(self, stage, x, rois, object_feats, img_metas):
        ...
        if self.angle_version == 'v1':
            ...
            _rois[:, -1] = -_rois[:, -1]
            bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs], _rois)
        else:
        ...
        proposal_list = self.bbox_head[stage].refine_bboxes(
            ...
            img_metas)  # len(4) (num_proposal, 4)
        bbox_results = dict(
            cls_score=cls_score,
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            ...
            for i in range(num_imgs):
                normalize_bbox_ccwh = torch.cat((proposal_list[i][:,:-1] / imgs_whwh[i], proposal_list[i][:,[-1]]), dim=-1)
    ...
    def simple_test(self,
        ...
        for stage in range(self.num_stages):
            ...
            proposal_list = bbox_results['detach_proposal_list']
        # ######################################### 以下用于调试
        # cls_score = torch.rand((proposal_list[0].shape[0], 15)).to(proposal_list[0])[None]
        # proposal_list = torch.cat((proposal_list[0][:,:-1], -1.57 * torch.ones((proposal_list[0].shape[0], 1)).to(proposal_list[0])), dim=-1)[None]
        #
        # ######################################################
        ...
        for img_id in range(num_imgs):
            ...
            #                                           num_classes]
            #############################################################
            cls_score_per_img, _labels = cls_score[img_id].max(-1)
            pos_inds = cls_score_per_img > 0.05
            labels_per_img = _labels[pos_inds]
            bbox_pred_per_img = proposal_list[img_id][pos_inds]
            scores_per_img = cls_score_per_img[pos_inds]
        ...
        bbox_results = [
            rbbox2result(det_bboxes[i], det_labels[i], num_classes)
            for i in range(num_imgs)
        ]
