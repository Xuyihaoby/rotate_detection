...
from mmdet.models.utils.myutils import Reduce_Sum
...
class ConvFCMultiLvlsWithOriginalImageSingleMaskBBoxHead(BBoxHead):
    ...
    def init_weights(self):
        ...
        if self.fc_ori_img:
            nn.init.xavier_uniform_(self.fc_ori_img.weight)
            nn.init.constant_(self.fc_ori_img.bias, 0)
        if self.with_cls:
    ...
    def forward(self, x):
        ...
        # x: [P2, P3, P4, P5, seg_fea, (original image)], P2~P5 are [K', 256, 7, 7], last is [K, 3, 7, 7]
        assert len(x) == 6
        # shared part
        ...
        if self.num_shared_fcs > 0:
            ...
            for xff in x[:-1]:
                xff = xff.flatten(1)
                x_from_feat.append(self.relu(self.shared_fcs[0](xff)))
            x0 = Reduce_Sum(x_from_feat)

            x_from_img = x[-1]
            x_from_img = x_from_img.flatten(1)
            x1 = self.relu(self.fc_ori_img(x_from_img))  # [K, 1024]
            x_sum = Reduce_Sum([x0, x1])
            x_shared = self.relu(self.shared_fcs[-1](x_sum))
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_hor_gt_bboxes,
        ...
        if num_pos > 0:
            ...
            if not self.reg_decoded_bbox:
                pos_hor_bbox_targets = self.bbox_coder.encode(pos_bboxes, pos_hor_gt_bboxes)
