...
class RDIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        for _ in range(num_reg_fcs):
            ...
            self.reg_fcs.append(
                build_activation_layer(dict(type='ReLU', inplace=True)))
        # over load the self.fc_cls in BBoxHead
        self.fc_reg = nn.Linear(in_channels, 5)   # 也就多了个检测头
    ...
    def forward(self, roi_feat, proposal_feat):
        ...
        # shape(bs*num_proposal, num_embedding)
        # roi shape (bs*num_proposal, embedding_dim, 7, 7)
        proposal_feat_iic = self.instance_interactive_conv(
            proposal_feat, roi_feat)
        # shape(bs * num_proposal, embedding_dim)
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                norm_pos_bbox_pred = torch.cat((pos_bbox_pred[:, :-1] / imgs_whwh, pos_bbox_pred[:, [-1]]), dim=-1)
                norm_pos_bbox_targets = bbox_targets[pos_inds.type(torch.bool)]
                norm_pos_bbox_targets = torch.cat((norm_pos_bbox_targets[:, :-1] / imgs_whwh,
                                                   norm_pos_bbox_targets[:, [-1]]), dim=-1)
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_meta):
        assert rois.size(1) == 5, repr(rois.shape)
        bboxes = self.bbox_coder.decode(rois, bbox_pred, max_shape=img_meta['img_shape'])
        return bboxes
