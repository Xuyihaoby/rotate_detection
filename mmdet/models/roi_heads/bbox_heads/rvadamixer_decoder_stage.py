...
from mmdet.core.bbox.samplers.sampling_3d_operator import sampling_3d
from mmdet.models.utils.adaptive_mixing_operator import AdaptiveMixing

from mmdet.core.bbox.iou_calculators import RBboxOverlaps2D
...
def rdecode_box(xyzr, delta):
    scale = 2.00 ** xyzr[..., 2:3]
    ratio = 2.00 ** torch.cat([xyzr[..., 3:4] * -0.5,
                               xyzr[..., 3:4] * 0.5], dim=-1)
    wh = scale * ratio
    xy = xyzr[..., 0:2]
    roi = torch.cat([xy, wh, xyzr[..., 4:5] + delta[..., 4:5]], dim=-1)
    return roi

def _decode_box(xyzr):
    scale = 2.00 ** xyzr[..., 2:3]
    ratio = 2.00 ** torch.cat([xyzr[..., 3:4] * -0.5,
                               xyzr[..., 3:4] * 0.5], dim=-1)
    wh = scale * ratio
    xy = xyzr[..., 0:2]
    roi = torch.cat([xy, wh, xyzr[..., 4:5]], dim=-1)
    return roi
...
class AdaptiveSamplingMixing(nn.Module):
    ...
    def forward(self, x, query_feat, query_xyzr, featmap_strides):
        # query_feat shape (bs, num_proposal, embedding_dim)
        # query_xyzr shape (bs, num_proposal, 4)
        # todo 5.12 根据query_feat reshape之后的维度修改offset generator的维度
        offset = self.sampling_offset_generator(query_feat)  # shape (bs, num_proposal, in_points * n_groups * 3)

        sample_points_xyz = make_sample_points(
            offset, self.n_groups * self.in_points,
            query_xyzr,
        )  # shape (bs, num_proposal, 1, in_points * n_groups. 3)
        # 这个函数的目的主要是为了将offset学习到的参数加到query xyz

        sampled_feature, _ = sampling_3d(sample_points_xyz, x,
                                         featmap_strides=featmap_strides,
                                         n_points=self.in_points,
                                         )
        # sampled_feature shape (bs, num_proposal, n_group, n_point, content_dim/group)
        # query_feat (num_bs)
        # todo 5.12 in this step should decide the choice of decoder
...
class RVAdaMixerDecoderStage(BBoxHead):
    _DEBUG = -1
    ...
    def __init__(self,
                 ...
                 version='v1',
                 **kwargs):
        super(RVAdaMixerDecoderStage, self).__init__(
        ...
        # iou calculater
        self.iou_calculater = RBboxOverlaps2D(version=version)
    ...
    def forward(self,
        ...
        iof = torch.zeros((0, n_query, n_query)).to(query_content)
        with torch.no_grad():
            rois = _decode_box(query_xyzr)
            roi_box_batched = rois.view(N, n_query, 5)
            for img_index in range(N):
                _iof = self.iou_calculater(roi_box_batched[img_index], roi_box_batched[img_index], mode='iof')
                iof = torch.cat((iof, _iof.unsqueeze(0)), dim=0)

            # iof = torch.stack((iof[:300, :300], iof[-300:, -300:]))[:, None, :, :]
            iof = iof[:, None, :, :]
            iof = (iof + 1e-7).log()
            pe = position_embedding(query_xyzr[..., :-1],
                                    query_content.size(-1) // 4)  # (bs, num_proposals, embedding_dim)
            # 这里如果类比乘decoder的话，讲道理有些奇怪，pe居然是需要自己算的

        '''IoF'''
        attn_bias = (iof * self.iof_tau.view(1, -1, 1, 1)).flatten(0, 1)  # (bs*num_head, num_proposal, num_proposal)

        query_content = query_content.permute(1, 0, 2)  # (num_proposal, bs, embedding_dim)
        pe = pe.permute(1, 0, 2)  # (num_proposal, bs, embedding_dim)
        ...
        # TODO 5.12 reshape (bs*num_head, numproposal, embedding_dim)
        query_content = self.attention_norm(query_content)
        query_content = query_content.permute(1, 0, 2)  # (bs, num_proposal, embed_dim)
        ...
        # shape (bs, num_proposal, embedding dim)
        # FFN
    ...
    def refine_xyzr(self, xyzr, xyzr_delta, return_bbox=True):
        ...
        new_theta = xyzr[..., 4:5] + xyzr_delta[..., 4:5]
        xyzr = torch.cat([new_xy, new_zr, new_theta], dim=-1)
        if return_bbox:
            return xyzr, rdecode_box(xyzr, xyzr_delta)
        else:
            return xyzr
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                norm_bbox_pred = torch.cat((pos_bbox_pred[:, :-1] / imgs_whwh, pos_bbox_pred[:, -1:]), dim=-1)
                pos_bbox_targets = bbox_targets[pos_inds.type(torch.bool)]
                norm_pos_bbox_targets = torch.cat((pos_bbox_targets[:, :-1] / imgs_whwh, pos_bbox_targets[:, -1:]),
                                                  dim=-1)
                losses['loss_bbox'] = self.loss_bbox(
                    norm_bbox_pred,
                    norm_pos_bbox_targets,
