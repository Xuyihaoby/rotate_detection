# modified from https://github.com/jbwang1997/OBBDetection
import torch
...
from mmdet.core import distance2rbbox, multi_apply, multiclass_nms_r, obb2poly, poly2obb
...
class RFCOSHead(AnchorFreeHead):
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            det_bboxes = self._get_bboxes_single(
                cls_score_list, bbox_pred_list, centerness_pred_list, theta_pred_list,
    ...
    def _get_bboxes_single(self,
        ...
        assert len(cls_scores) == len(bbox_preds) == len(mlvl_points) == len(theta_preds)
        mlvl_bboxes = []
        mlvl_scores = []
        mlvl_centerness = []
        for cls_score, bbox_pred, centerness, theta_pred, points in zip(
                cls_scores, bbox_preds, centernesses, theta_preds, mlvl_points):

            assert cls_score.size()[-2:] == bbox_pred.size()[-2:] == theta_pred.size()[-2:]
            ...
            bboxes = distance2rbbox(points, bbox_pred, version=self.version)
        ...
        # 本来只有15个类，但是为了便于后续的操作，所以需要再额外增加一个背景类，以便于模型后续的处理
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list):
        ...
        # .new_tensor主要目的是将尺度范围转到cuda上，之后再将尺度拓展成为point的尺度
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        # compute distance
        # modified from https://github.com/jbwang1997/OBBDetection
        # get count direction bbox ---> horizon bbox
        # 和原版相比不够优雅，但是比较适合不同角度定义修改
        gt_ctr, gt_wh, gt_thetas = torch.split(
            gt_bboxes, [2, 2, 1], dim=1)
        gt_poly = obb2poly(gt_bboxes, version=self.version)
        ...
        ymin = modified_poly[:, :, 1, :].min(dim=2, keepdim=True)[0]
        ymax = modified_poly[:, :, 1, :].max(dim=2, keepdim=True)[0]

        modified_gt_bboxes = torch.cat((xmin, ymin, xmax, ymax), dim=2)
        # modified_gt_bboxes = modified_gt_bboxes[None].expand(num_points, num_gts, 4)

        # origin not suitable for rotate bbox need counter direction
        xs, ys = points[:, 0], points[:, 1]
        xs = xs[:, None].expand(num_points, num_gts)
        ys = ys[:, None].expand(num_points, num_gts)

        left = xs - modified_gt_bboxes[..., 0]
        right = modified_gt_bboxes[..., 2] - xs
        top = ys - modified_gt_bboxes[..., 1]
        bottom = modified_gt_bboxes[..., 3] - ys
        bbox_targets = torch.stack((left, top, right, bottom), -1)

        if self.center_sampling:
            # 这一段AOPG同样使用了更简单的offset 坐标偏移公式去进行转换
            # condition1: inside a `center bbox`
            radius = self.center_sample_radius
            center_xs = (modified_gt_bboxes[..., 0] + modified_gt_bboxes[..., 2]) / 2
            center_ys = (modified_gt_bboxes[..., 1] + modified_gt_bboxes[..., 3]) / 2
            center_gts = torch.zeros_like(modified_gt_bboxes)  # shape (num_points, num_gts, 4)
            stride = center_xs.new_zeros(center_xs.shape)  # shape (num_points, 2)

            # project the points on current lvl back to the `original` sizes
            lvl_begin = 0
            for lvl_idx, num_points_lvl in enumerate(num_points_per_lvl):
                lvl_end = lvl_begin + num_points_lvl
                stride[lvl_begin:lvl_end] = self.strides[lvl_idx] * radius  # 给每一个层级对应的点分配尺度
                lvl_begin = lvl_end
            ...
            center_gts[..., 0] = torch.where(x_mins > modified_gt_bboxes[..., 0],
                                             x_mins, modified_gt_bboxes[..., 0])
            center_gts[..., 1] = torch.where(y_mins > modified_gt_bboxes[..., 1],
                                             y_mins, modified_gt_bboxes[..., 1])
            center_gts[..., 2] = torch.where(x_maxs > modified_gt_bboxes[..., 2],
                                             modified_gt_bboxes[..., 2], x_maxs)
            center_gts[..., 3] = torch.where(y_maxs > modified_gt_bboxes[..., 3],
                                             modified_gt_bboxes[..., 3], y_maxs)
        ...
        # 其次对应点只有在对应的尺度上可以赋值
        # shape (num_points, num_gts)
        # areas shape (num_points, num_gts)
        ...
        theta_targets = gt_thetas[None].expand(num_points, num_gts, 1)[range(num_points), min_area_inds]
