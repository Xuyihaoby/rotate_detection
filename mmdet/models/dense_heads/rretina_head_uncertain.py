...
class RRetinaHeadUncertain(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        super(RRetinaHeadUncertain, self).__init__(
            num_classes,
            in_channels,
    ...
    def _init_layers(self):
        ...
        self.uncertain_convs = nn.ModuleList()
        ...
        # add uncertain conv
        self.retina_uncertain = nn.Conv2d(
            self.feat_channels, 1, 1
        )
        self.uncertain_relu = nn.ReLU(inplace=True)
    ...
    def init_weights(self):
        ...
        for m in self.reg_convs:
            normal_init(m.conv, std=0.01)
        for m in self.uncertain_convs:
            normal_init(m.conv, std=0.01)
        bias_cls = bias_init_with_prob(0.01)
        normal_init(self.retina_cls, std=0.01, bias=bias_cls)
        normal_init(self.retina_uncertain, std=0.01)
    ...
    def forward_single(self, x):
        ...
        # add uncertain conv
        for uncertain_conv in self.uncertain_convs:
            uncertain_feat = uncertain_conv(uncertain_feat)
        cls_score = self.retina_cls(cls_feat)
        bbox_pred = self.retina_reg(reg_feat)

        # add uncertain
        uncetain_scalar = self.relu(self.retina_uncertain(uncertain_feat))
        return cls_score, bbox_pred, uncetain_scalar

    def loss_single(self, cls_score, bbox_pred, uncertain_scalar, anchors, labels, label_weights,
        ...
        ## add uncertain
        uncertain_scalar = uncertain_scalar.repeat(1, self.num_anchors, 1, 1).permute(0, 2, 3, 1).reshape(-1)

        # [batchszie, channel, H, W] --> [batchszie, H, W, channel] --> [., c]
        _uncertain_cls_weights = label_weights * (-1 * uncertain_scalar).exp()
        _uncertain_cls_bias = label_weights * 0.3 * uncertain_scalar
        loss_cls = self.loss_cls(cls_score, labels, _uncertain_cls_weights,
                                 # avg_factor=num_total_samples,
                                 reduction_override='none')

        loss_cls = (loss_cls + _uncertain_cls_bias[..., None]).sum() / num_total_samples
        ...
        _uncertain_reg_weights = (bbox_weights.mean(-1) * (-1 * uncertain_scalar).exp())[..., None].repeat(1, 5)
        _uncertain_reg_bias = bbox_weights.mean(-1) * 0.3 * uncertain_scalar
        loss_bbox = self.loss_bbox(
            bbox_pred,
            bbox_targets,
            _uncertain_reg_weights,
            # avg_factor=num_total_samples,
            reduction_override='none')
        loss_bbox = (loss_bbox + _uncertain_reg_bias).sum() / num_total_samples
...
@HEADS.register_module()
class RRetinaHeadUncertainSep(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        super(RRetinaHeadUncertainSep, self).__init__(
            num_classes,
            in_channels,
            anchor_generator=anchor_generator,
            **kwargs)
    ...
    def _init_layers(self):
        ...
        self.retina_reg = nn.Conv2d(
            self.feat_channels, self.num_anchors * 5, 3, padding=1)
        # add uncertain conv
        self.retina_reg_uncertain = nn.Conv2d(
            self.feat_channels, 1, 1
        )
        self.retina_cls_uncertain = nn.Conv2d(self.feat_channels, 1, 1)
        self.uncertain_relu = nn.ReLU(inplace=True)
    ...
    def init_weights(self):
        ...
        bias_cls = bias_init_with_prob(0.01)
        normal_init(self.retina_cls, std=0.01, bias=bias_cls)
        normal_init(self.retina_reg_uncertain, std=0.01)
        normal_init(self.retina_cls_uncertain, std=0.01)
        normal_init(self.retina_reg, std=0.01)
    ...
    def forward_single(self, x):
        ...
        # add uncertain
        uncertain_reg_scalar = self.relu(self.retina_reg_uncertain(reg_feat))
        uncertain_cls_scalar = self.relu(self.retina_cls_uncertain(cls_feat))
        return cls_score, bbox_pred, uncertain_reg_scalar, uncertain_cls_scalar

    def loss_single(self, cls_score, bbox_pred,
                    uncertain_reg_scalar, uncertain_cls_scalar,
        ...
        ## add uncertain
        uncertain_cls_scalar = uncertain_cls_scalar.repeat(1, self.num_anchors, 1, 1).permute(0, 2, 3, 1).reshape(-1)

        # [batchszie, channel, H, W] --> [batchszie, H, W, channel] --> [., c]
        _uncertain_cls_weights = label_weights * (-1 * uncertain_cls_scalar).exp()
        _uncertain_cls_bias = label_weights * 0.3 * uncertain_cls_scalar
        loss_cls = self.loss_cls(cls_score, labels, _uncertain_cls_weights,
                                 # avg_factor=num_total_samples,
                                 reduction_override='none')
        ...
        uncertain_reg_scalar = uncertain_reg_scalar.repeat(1, self.num_anchors, 1, 1).permute(0, 2, 3, 1).reshape(-1)
        _uncertain_reg_weights = (bbox_weights.mean(-1) * (-1 * uncertain_reg_scalar).exp())[..., None].repeat(1, 5)
        _uncertain_reg_bias = bbox_weights.mean(-1) * 0.3 * uncertain_reg_scalar
        loss_bbox = self.loss_bbox(
            bbox_pred,
            bbox_targets,
            _uncertain_reg_weights,
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'uncetain_reg_scalars', 'uncetain_cls_scalars'))
    def loss(self,
             cls_scores,
             bbox_preds,
             uncetain_reg_scalars,
             uncetain_cls_scalars,
             gt_bboxes,
             gt_labels,
             img_metas,
             gt_bboxes_ignore=None):
    ...
    def get_bboxes(self,
                   cls_scores,
                   bbox_preds,
                   uncertain_reg_scalars,
                   uncertain_cls_scalars,
                   img_metas,
                   cfg=None,
                   rescale=False,
                   with_nms=True):
