...
from mmdet.core import (images_to_levels, multi_apply, unmap, ranchor_inside_flags,
                        multiclass_nms_r, MlvlPointGenerator, reduce_mean)
...
class RRetinaHeadOTA(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        super(RRetinaHeadOTA, self).__init__(
            num_classes,
            in_channels,
            anchor_generator=anchor_generator,
            **kwargs)
        self.prior_generator = MlvlPointGenerator([8, 16, 32, 64, 128], offset=0)
    ...
    def loss(self,
        ...
        _cls_scores = self.levels2images(cls_scores)
        _bbox_preds = self.levels2images(bbox_preds)
        with torch.no_grad():
    ...
    def levels2images(self, mlvl_tensor):
        ...
        for t in mlvl_tensor:
            if t.dim() == 4:
                t = t.permute(0, 2, 3, 1)
                t = t.view(batch_size, -1, channels).contiguous().detach()  # 要加上detach()
    ...
    def _get_targets_single(self,
        ...
        decoded_bboxes = self.bbox_coder.decode(flat_anchors, bbox_preds)
        ...
        decoded_bboxes = decoded_bboxes[inside_flags, :]
        cls_scores = cls_scores[inside_flags, :]
        # import pdb
        # pdb.set_trace()
        assign_result = self.assigner.assign(cls_scores.sigmoid(),
            anchors, decoded_bboxes, gt_bboxes, gt_labels, gt_bboxes_ignore)
