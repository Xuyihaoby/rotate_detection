...
class RTransformerHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.theta = Linear(self.embed_dims, 1)
    ...
    def forward_single(self, x, img_metas):
        ...
        # self.query_embedding.weight shape (100, 256)
        # mask shape (bs, h, w)
        outs_dec, _ = self.transformer(x, masks, self.query_embedding.weight,
                                       pos_embed)
        # due to return intermediate feature
        # so shape (num_decoder layer, bs, 100, embedding_heads)
        all_cls_scores = self.fc_cls(outs_dec)
        all_bbox_preds = self.fc_reg(self.activate(
            self.reg_ffn(outs_dec))).sigmoid()

        all_bbox_theta = self.theta(self.activate(
            self.reg_ffn(outs_dec)))
        all_bbox_theta = torch.where(all_bbox_theta >= 0, all_bbox_theta - torch.tensor(np.pi / 2), all_bbox_theta)
        all_bbox_theta = torch.where(all_bbox_theta < torch.tensor(-np.pi / 2),
                                     all_bbox_theta + torch.tensor(np.pi / 2), all_bbox_theta)
        all_bbox_preds = torch.cat((all_bbox_preds, all_bbox_theta), dim=-1)
    ...
    def loss_single(self,
        ...
        """
        num_imgs = cls_scores.size(0)  # shape (bs, 100\query embeding, numclass+1,)
    ...
    def get_targets(self,
        ...
        # 返回的格式和传入的格式一样
        # 每个元素的shape有区别,第一个参数也总是query_embedding
    ...
    def _get_target_single(self,
        ...
        """
        # 注意，这一个single内部只对一张图片的单个decoder进行处理，并且得到的正样本数总是和gt bboxes的数量匹配
        num_bboxes = bbox_pred.size(0)
        # assigner and sampler
        assign_result = self.assigner.assign(bbox_pred, cls_score, gt_bboxes,
                                             gt_labels, img_meta,
                                             gt_bboxes_ignore)
        # 匈牙利匹配算法的大致流程就是算完几个cost之后进行相加后得到维度（num_bbox\num_query_embedding, num_gt）的矩阵
        # 然后通过匹配选择出总cost最小成为正样本
        ...
        pos_gt_bboxes_targets = torch.cat((pos_gt_bboxes_normalized, sampling_result.pos_gt_bboxes[:, [-1]]), dim=-1)
    ...
    def _get_bboxes_single(self,
        ...
        score_flag = self.test_cfg.get('use_score', False)

        # exclude background
        scores, det_labels = F.softmax(cls_score, dim=-1)[..., :-1].max(-1)

        if score_flag:
            pos_inds = scores > score_thr
            bbox_pred = bbox_pred[pos_inds]
            scores = scores[pos_inds]
            det_labels = det_labels[pos_inds]
            if len(det_labels) == 0:
                bbox_pred_ = torch.cat((bbox_pred, scores.unsqueeze(1)), -1)
                return bbox_pred_, det_labels

        scores, index = scores.topk(max_per_img)
        det_bboxes = bbox_pred[index]
        det_labels = det_labels[index]

        det_bboxes[:, 0:4] = det_bboxes[:, 0:4] * img_shape[1]
        # det_bboxes[:, 1::2] = det_bboxes[:, 1::2] * img_shape[0]
        det_bboxes[:, 0:4].clamp_(min=0, max=img_shape[1])
        if rescale:
