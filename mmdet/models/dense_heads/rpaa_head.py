...
from mmdet.core import multi_apply, multiclass_nms_r
# from mmdet.core.bbox.iou_calculators import bbox_overlaps
from mmdet.core.bbox.iou_calculators import RBboxOverlaps2D
from mmdet.models import HEADS
# from mmdet.models.dense_heads import ATSSHead
from mmdet.models.dense_heads import RRetinaHeadATSS
from mmdet.models.dense_heads import RRetinaHead
EPS = 1e-12
try:
...
class RPAAHeadVanilla(RRetinaHead):
    ...
    def __init__(self,
        ...
        self.bbox_overlap = RBboxOverlaps2D(angle_version)
        self.topk = topk
        self.with_score_voting = score_voting
        self.covariance_type = covariance_type
        super(RPAAHeadVanilla, self).__init__(*args, **kwargs)
    ...
    def loss(self,
        ...
        bbox_preds = [item.reshape(-1, 5) for item in bbox_preds]
    ...
    def _get_targets_single(self,
        ...
        return super(RPAAHeadVanilla, self)._get_targets_single(
            flat_anchors,
            valid_flags,
            gt_bboxes,
            gt_bboxes_ignore,
    ...
    def score_voting(self, det_bboxes, det_labels, mlvl_bboxes,
        ...
        """
        # 该方法从形式上来说基本上很想soft nms
        # 但是它实际上是想通过给定候选区域，把所有符合的区域都找出来，然后利用概率分布的方式重新进行计算
        # mlvl_nms_score 是针对预测的score与iou进行处理过的得分，未经nms shape (N, clsnum+1)
        # mlvl_bboxes (N, 5) N是before nms 的结果
        candidate_mask = mlvl_nms_scores > score_thr  # (N, clsnum+1)
        candidate_mask_nozeros = candidate_mask.nonzero()
        candidate_inds = candidate_mask_nozeros[:, 0]  # (n, 1)符合条件的个数
        candidate_labels = candidate_mask_nozeros[:, 1]
        candidate_bboxes = mlvl_bboxes[candidate_inds]
        candidate_scores = mlvl_nms_scores[candidate_mask]
        # 以上基本就是把before nms之前符合大于阈值的点全部选出来
        det_bboxes_voted = []
        det_labels_voted = []
...
# 这个是针对ATSS的
@HEADS.register_module()
class RPAAHead(RRetinaHeadATSS):
    """Head of PAAAssignment: Probabilistic Anchor Assignment with IoU
    Prediction for Object Detection.
    ...
    def __init__(self,
        ...
        self.with_score_voting = score_voting
        self.covariance_type = covariance_type
        super(RPAAHead, self).__init__(*args, **kwargs)
    ...
    def _get_targets_single(self,
        ...
        assert unmap_outputs, 'We must map outputs back to the original' \
            'set of anchors in PAAhead'
        return super(RPAAHead, self)._get_targets_single(
            flat_anchors,
            valid_flags,
            num_level_anchors,
            gt_bboxes,
