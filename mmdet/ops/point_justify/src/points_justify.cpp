...
int PointsJFAlignLaucher(const at::Tensor points, const at::Tensor polygons,
                const int rows, const int cols, at::Tensor output);
...
int pointsJfAlign(at::Tensor points, at::Tensor polygons, at::Tensor output) {
    CHECK_INPUT(points);
    CHECK_INPUT(polygons);
    CHECK_INPUT(output);
    ...
    PointsJFAlignLaucher(points, polygons, rows, cols, output);
    return 1;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("pointsJfAlign", &pointsJfAlign, "Justify a point is in polygon align or not");  //该对象对应的方法名   函数指针  说明
}