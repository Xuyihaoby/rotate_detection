...
from . import torch_batch_svd_cpu
...
class BatchSVDFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        torch_batch_svd_cpu.batch_svd_forward(input, U, S, V, True, 1e-7, 100, is_double)
        U.transpose_(1, 2)
        V.transpose_(1, 2)
        if ctx.is_half:
    ...
    def backward(ctx, grad_u: torch.Tensor, grad_s: torch.Tensor,
        ...
        grad_out: torch.Tensor = torch_batch_svd_cpu.batch_svd_backward(
