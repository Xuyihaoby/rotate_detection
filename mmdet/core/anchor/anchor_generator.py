...
class AnchorGenerator(object):
    ...
    def _meshgrid(self, x, y, row_major=True):
        ...
        xx = x.repeat(len(y))
        yy = y.view(-1, 1).repeat(1, len(x)).view(-1)  # 1dim  --> 2dim --> 2dim --> 1dim
        # etc: (size) [len(y)] --> [len(y), 1] --> [len(y), len(x)] --> [len(y)*len(x)]
        if row_major:
            return xx, yy
        else:
            return yy, xx
    ...
    def single_level_grid_anchors(self,
        ...
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1)  # [len(x)*len(y), 4]
