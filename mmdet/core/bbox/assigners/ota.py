...
class OTAAssigner(BaseAssigner):
    ...
    def _assign(self,
        ...
        # shape (num_valid, num_gt, num_cls)
        ...
        cost_matrix = (
                ...
                (~is_in_boxes_and_center.int()) * INF)

        matched_pred_ious, matched_gt_inds = \
            self.dynamic_k_matching(cost_matrix, pairwise_ious, num_gt, valid_mask)
    ...
    def get_in_gt_and_in_center_info(self, priors, gt_bboxes):
        ...
        is_in_gts = torch.full((num_priors, num_gt), 0).to(gt_bboxes)
        gt_polygons = obb2poly(gt_bboxes, self.angle_version)
        from mmdet.ops import pointsJf
        pointsJf(priors[:, :2].contiguous(), gt_polygons, is_in_gts)
        is_in_gts_all = is_in_gts.max(dim=1).values > 0

        # both in boxes and centers, shape: [num_fg, num_gt]
        is_in_boxes_and_centers = (
            is_in_gts[is_in_gts_all, :])
        return is_in_gts_all, is_in_boxes_and_centers

    # TODO 将选择中心点改成离中心点最近的k个点
