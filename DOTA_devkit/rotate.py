...
def rotate_single_run(name, srcpath, dstpath):
    ...
    for stem in objs:
        objname += [stem['name']]
    if ('bridge' in objname or 'soccer-ball-field' in objname or 'roundabout' in objname or 'helicopter' in objname):
        print(objname)
...
def rotate(srcpath, dstpath):
    ...
    # for name in names:
    #     rotate_single_run(name, srcpath=srcpath, dstpath=dstpath)

if __name__ == '__main__':
    rotate(r'/data/xuyihao/mmdetection/dataset/DOTA/train_single',
