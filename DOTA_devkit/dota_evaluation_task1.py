...
def voc_eval(detpath,
    ...
    nd = len(image_ids)  # 图片数量 与其说是图片数量倒不如说是单一种类所检测出的所有方框的数量更加合适
    tp = np.zeros(nd)
    fp = np.zeros(nd)
    for d in range(nd):
...
def main():
    ...
    detpath = r'/data/xuyihao/mmdetection/mycode/submisssion_fasterrcnn_r_nms/Task1_{:s}.txt'
    annopath = r'/data/xuyihao/mmdetection/dataset/DOTA/val/labelTxt/{:s}.txt'  # change the directory to the path of val/labelTxt, if you want to do evaluation on the valset
    imagesetfile = r'/data/xuyihao/mmdetection/dataset/DOTA/val.txt'
